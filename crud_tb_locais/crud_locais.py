...
from c_locais import CadastrarLocal
from u_locais import AlterarBlocos
from d_locais import ExcluirLocais
...
class CRUDLocais(tk.Tk):
    def __init__(self):
        ...
        self.tre_locais = ttk.Treeview(self, columns=("idt_local", "nme_local", "nme_campus"),
                                       show="headings",
                                       style="Custom.Treeview")
        # Configurar as colunas
        self.tre_locais.heading("idt_local", text="Id")
        self.tre_locais.heading("nme_local", text="Nome do Local")
        self.tre_locais.heading("nme_campus", text="Campus")
        # Ajustar a largura das colunas
        self.tre_locais.column("idt_local", width=50, anchor="center")
        self.tre_locais.column("nme_local", width=300)
        self.tre_locais.column("nme_campus", width=200, anchor="center")
        self.tre_locais.grid(row=3, column=0, columnspan=6, padx=self.PADX, pady=self.PADY)
        ...
        self.bt_alterar = tk.Button(frame_botoes, text="Alterar", command=self.alterar, font='Helvetica 12 bold',
                                    fg='white', bg=self.ROXO)
        self.bt_alterar.grid(row=0, column=2, padx=self.PADY, pady=self.PADY, sticky="ew")
    ...
    def consultar(self):
        ...
        cmd = (
            "SELECT l.idt_local, l.nme_local, COALESCE(c.nme_campus, 'Sem Campus') AS nme_campus "
            "FROM tb_locais l "
            "LEFT JOIN tb_campus c ON l.cod_campus = c.idt_campus "
            "WHERE l.nme_local LIKE CONCAT('%', %s, '%') ORDER BY l.nme_local"
        )
        locais = self.sql.get_list(cmd, [nome])

        self.limpar_tabela()
        for local in locais:
            self.tre_locais.insert("", tk.END, values=(local['idt_local'], local['nme_local'], local['nme_campus']))
    ...
    def cadastrar(self):
        CadastrarLocal(self)  # Mudança aqui
